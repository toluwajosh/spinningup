...
def setup_logger_kwargs(exp_name, seed=None, data_dir=None, datestamp=False):
    ...
    logger_kwargs = dict(
        output_dir=osp.join(data_dir, relpath), exp_name=exp_name
    )
...
def call_experiment(
    ...
    **kwargs,
    ...
    print(
        json.dumps(
            kwargs_json, separators=(",", ":\t"), indent=4, sort_keys=True
        )
    )
    print("\n")
    ...
    test_cmd = (
        "python -m spinup.run test_policy " + logger_kwargs["output_dir"]
    )
...
class ExperimentGrid:
    ...
    def variant_name(self, variant):
        ...
        for k, v, sh, inn in zip(
            self.keys, self.vals, self.shs, self.in_names
        ):
